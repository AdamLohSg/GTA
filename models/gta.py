...
class AdaGCNConv(MessagePassing):
    ...
    def build_edge_idx(self):
        # Initialize edge index matrix
        E = torch.zeros(
            (2, self.num_nodes * (self.num_nodes - 1)), dtype=torch.long)

        # Populate 1st row
        for node in range(self.num_nodes):
            for neighbor in range(self.num_nodes - 1):
                E[0, node * (self.num_nodes - 1) + neighbor] = node

        # Populate 2nd row
        neighbors = []
        for node in range(self.num_nodes):
            neighbors.append(list(np.arange(node)) +
                             list(np.arange(node+1, self.num_nodes)))
        E[1, :] = torch.Tensor(
            [item for sublist in neighbors for item in sublist])
...
class GraphTemporalEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.gc_module(0)(x.transpose(0, 1), self.edge_index).transpose(
            0, 1)  # >> (bsz, num_nodes, seq_len)
        # output = x
...
class GTA(torch.nn.Module):
    ...
    def forward(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        dec_inp = torch.zeros_like(
            batch_y[:, -self.out_len:, :]).double().to(self.device)
        dec_inp = torch.cat([batch_y[:, :self.label_len, :],
                            dec_inp], dim=1).double().to(self.device)
